...
4. **Guarda de código (anti-regressão):** ✅ **implementada em 2026-08-17** — `Candle::is_degenerate()` no domínio; o live persiste a barra flat para auditoria mas não a deixa alimentar estratégia/gestão de posição; 5 seguidas → evento `degenerate_bar_streak` em `system_events` + alerta crítico no webhook. O `ingest` conta e alerta barras degeneradas. Efeito prático enquanto o feed estiver degradado: o live fica essencialmente inerte (a maioria das barras é flat) — proteção intencional.
