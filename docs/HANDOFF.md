...
0. **⚠️ CRÍTICO — feed de dados degradado na VM (descoberto em 2026-08-17, ver `docs/reports/validacao-live-vs-backtest-2026-08-07_a_08-14.md`):** desde a migração (08-07), as barras recentes chegam à VM como 1 print (high==low, volume 0) — a conta paper não tem subscrição de dados em tempo real. O backtest não reproduz o live dos dias 4–9 por causa disso; a amostra do gate B do período está contaminada (decisão do dono pendente sobre os trades 10 e 11). **Correção do feed:** habilitar compartilhamento de market data com a conta paper no portal IBKR (ou assinar dados) — ação manual do dono no Client Portal. **Mitigações de código já implementadas (2026-08-17):** guarda anti-barra-degenerada no live com **espera de consolidação** (o feed entrega a barra recém-fechada como 1 print e consolida ~3–4 min depois — o cursor espera em vez de operar a barra flat; desiste em ~15 min e alerta), **upsert de auto-reparo no repositório de candles** (só sobrescreve linha degenerada — banco se corrige quando a versão consolidada chega), contagem de barras degeneradas no `ingest`, e o comando `trader-cli debug-candles` para diagnóstico cru do feed. **Nota:** o dono habilitou "share market data" no portal em 2026-08-17 ~14h20 ET (Gateway reiniciado) — barras consolidadas vêm corretas, mas a barra recém-fechada segue chegando como 1 print; a espera de consolidação cobre isso. **Falta:** reparar o histórico (rodar `ingest` para 08-07 em diante — o upsert substitui as linhas flat) e refazer a validação cruzada.
