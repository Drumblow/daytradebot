...
- **07/09/2026 — ADR-019 (harness de validacao) IMPLEMENTADO, menos o item 8.** O defeito central era que `analyze` escolhia o backtest de referencia com `latest_by_strategy`: o run mais recente da estrategia, em QUALQUER simbolo e QUALQUER config — bastava rodar uma variacao de parametro para ela virar baseline do gate B em silencio. Agora e `latest_for(estrategia, par, config_hash)` ignorando runs `experimental`, e sem run compativel ele AVISA em vez de pegar outro. O lado live tambem passou a filtrar por `strategy_id`+`config_hash` (num simbolo com duas instancias a amostra misturava estrategias). `walkforward` ganhou `--output`, `--slippage-bps` (Decimal), `--label`, `--holdout-from`, `--strategy-config` e `--set chave=valor`, com travas que ABORTAM: `--set` sem `--label`; chave inexistente (os 9 `StrategyParameters` ganharam `deny_unknown_fields`); `--set` que nao muda o `config_hash`; `--set` junto de `--holdout-from`; TOML de `--strategy-config` com id diferente do `--strategy`; `--holdout-from` invalido e ERRO, nao holdout desligado em silencio. Metricas novas: **PF em R**, t-stat do avg R, corr(risco,R), quebra por direcao e por hora **ET**, concentracao por dia e por mes, custo total. O trade do backtest deixou de nascer anonimo (identidade + `market_snapshot` viajam Order->Position->journal). **NAO foi feito o dedupe de `backtest_runs`**: a auditoria mediu que no maior grupo "duplicado" (24 linhas) ha SETE valores distintos de `final_equity` — nao sao copias, e deduplicar apagaria resultados diferentes entre si; a migracao 0005 cria so o indice de busca e explica isso. **Achado do PF em R:** a balance-area em AVUV tem **PF_R 0,74** e em VBR **0,97** — em unidades de risco ela PERDE; o PF em dolares acima de 1 vem da correlacao entre tamanho e resultado (0,57 em AVUV), que e o cap de notional pondo posicao maior nos trades de stop largo. So a fade em SLYV passa no criterio de concentracao (59% contra teto de 60%); nenhum t-stat chega a 2. **Sob o gate proposto no ADR-019 §7 nenhuma das sete combinacoes passaria** — por isso os criterios novos sao impressos como PROPOSTA, separados do veredito do ADR-010, para o dono decidir (§10.2 do plano). Pendente: item 8 (relatorio em Python, `trader-research/`), sem o qual o criterio "IC95 em blocos >= 1,0" nao tem como ser avaliado.
