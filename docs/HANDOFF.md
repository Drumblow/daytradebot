...
3. **Webhook de alertas: mecanismo pronto, testado e PLUMBADO no app (2026-08-29)** — o entrypoint do bot carrega `${APP_DATA_DIR}/secrets/alerts.env` (criado pelo pre-install com placeholder; montado read-only nas 11 instâncias). Falta SÓ o dono colar a URL do webhook (Slack/Discord/Teams) nesse arquivo no servidor e reiniciar o app. Sem isso, um CB na madrugada segue mudo.
