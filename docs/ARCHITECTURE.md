...
| ADR-019 | Harness de validação e gate A estatístico | `--strategy-config`/`--set` permitem ablação sem código novo; holdout travado por `--holdout-from`; baseline do `analyze` por (estratégia, ativo, `config_hash`); métricas de dispersão e concentração. Implementado em parte: vários itens do §4/§6 não entraram (relatório em Python, Sharpe diário, `n_trials`, dedupe e outros). A lista e a contagem canônicas ficam na seção "Pendente" do próprio ADR-019 — não são replicadas aqui, porque cópias dessa lista já divergiram entre si. **Os critérios do §7 são proposta ainda não vigente — o gate em vigor continua sendo o do ADR-010.** |
