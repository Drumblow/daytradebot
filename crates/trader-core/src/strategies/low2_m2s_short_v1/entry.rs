//! Regras de entrada, stop e alvo para a estratégia Low 2 / M2S Short v1.

use rust_decimal::Decimal;
use serde_json::json;

use crate::strategies::low2_m2s_short_v1::setup::Setup;
...
pub fn build_signal(
    ...
    Signal {
        ...
        risk_amount: None,
        risk_percent: risk_per_trade_pct.map(|pct| pct / Decimal::from(100)),
        position_size: None,
        entry_reason: Some("low 2 / m2s pullback em tendência de baixa".to_string()),
...
mod tests {
    ...
    fn make_setup() -> Setup {
        Setup {
            ...
            entry_price: Decimal::from(99),
            stop_price: Decimal::from(100),
            target_price: Decimal::from(97),
        }
    }
